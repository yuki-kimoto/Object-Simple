package Object::Simple::Old;

use strict;
use warnings;
use Carp 'croak';

use Object::Simple::OldUtil;
use Object::Simple::Util;

use constant OldUtil => 'Object::Simple::OldUtil';
...
sub import {
    ...
    OldUtil->class_infos->{$caller_class}{base} = $options{base};
    
    # Regist mixin classes to meta information
    OldUtil->class_infos->{$caller_class}{mixins} = $options{mixins};
    ...
    OldUtil->define_MODIFY_CODE_ATTRIBUTES($caller_class);
...
sub new {
    ...
    # Class infos
    my $class_infos = OldUtil->class_infos;
    ...
    foreach my $super_class (@{OldUtil->get_leftmost_isa($class)}) {
...
sub build_class {
    ...
    while (my $accessor_info = shift @ACCESSOR_INFOS) {
        ...
        OldUtil->check_accessor_option($accessor_name, $class, $accessor_options,
                                     $accessor_type);
    ...
    foreach my $class (@build_need_classes) {
        ...
        OldUtil->include_mixin_classes($class)
        ...
        foreach my $accessor_name (keys %$accessors) {
            ...
            while ($class_infos->{$base_class}{accessors}{$accessor_name}{options}{extend}) {
                my ($super_accessor_options, $accessor_found_class)
                  = OldUtil->get_super_accessor_options($base_class, $accessor_name);
            ...
            my $code = $accessor_type eq 'Attr'
                     ...
                     ? OldUtil->create_output_accessor($class, $accessor_name, $options)
                     
                     # (Deprecated)
                     : $accessor_type eq 'Translate'
                     ? OldUtil->create_translate_accessor($class, $accessor_name, $options)
    ...
    # Create constructor
    foreach my $class (@build_need_classes) {
        my $constructor_code = OldUtil->create_constructor($class);
        
        eval $constructor_code;
        croak("$constructor_code\n:$@") if $@; # never occured
        
        $class_infos->{$class}{constructor}
          = \&{"Object::Simple::Old::Constructors::${class}::new"};
    }
...
sub class_attrs       { OldUtil->class_attrs(@_) }
sub exists_class_attr { OldUtil->exists_class_attr(@_) }
sub delete_class_attr { OldUtil->delete_class_attr(@_) }

=head1 NAME

Object::Simple::Old - a simple class builder(now depricated)
...
=head1 SYNOPSIS
    ...
    # Inheritance
    package Magazine;
    use Object::Simple::Old(base => 'Book');
...
Object::Simple::Old defines a 'new' method for the subclass, so you do not need
...
You can specify class name if you need.
    ...
    Object::Simple::Old->build_class($class);
...
You can resist accessor informations.
    ...
    Object::Simple::Old->resist_accessor_info($class, $accessor_name, 
                                         $accessor_options, $accessor_type);

The following is arguments sample

    Object::Simple::Old->resist_accessor_info('Book', 'title', {default => 1}, 'Attr');
...
Object::Simple::Old do not support multiple inheritance because it is so complex.
 
=head1 Mixin
 
Object::Simple::Old support mixin syntax
...
Object::Simple::Old mixin merge mixin class attribute.
    ...
    # using mixin class
    package Some::Class;
    use Object::Simple::Old(mixins => ['Some::Mixin']);
